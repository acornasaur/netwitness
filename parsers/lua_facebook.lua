	...
	if protocol == 6 then
		...
		if requestStream then
			if fbresp == 1 then
				...
				if not (self.state.identified or self.state.notFB) then
...
function lua_facebook:req2(token, first, last)
	if first <= 10 then
		local protocol, srcPort, dstPort  = nw.getTransport()
		if protocol == 17 and srcPort > 1024 then
			local status, error = pcall(function()
			self.state = self.state or {}
			if not (self.state.identified or self.state.notFB) then
				local service = nw.getAppType()
...
lua_facebook:setCallbacks({
	...
	["\250\206\176\0\85"] = lua_facebook.req2, -- fa ce b0 00 55 
})
