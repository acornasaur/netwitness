local rootdomain = nw.createParser("lua_rootdomain", "Get the root domain from alias.host meta")

--[[
    DESCRIPTION

        Parse the root domain from alias.host


    VERSION
		
		2017-07-05 - added 'local' to the tld table
		2017-06-08 - Updated TLD list and added dyndns entries
		2016-08-31 - Excluding NetBIOS as those are really just hostnames anyways
		2016-07-21 - added new check ip function
		2016-07-20 - added check for the host is actually an IP address
		2015-02-25 - added check and table for US State domains
        2015-02-14 - Initial development
    ...
    Be sure to normalize to lower case first    
...
rootdomain:setKeys({
    nwlanguagekey.create("root.host"),
})
...
function rootdomain:hostMeta(index, host)
	if host then
		...
		if app == 137 or app == 138 or app == 139 then
			...
			if ipcheck == true then
				nw.createMeta(self.keys["root.host"], host)
				--nw.logInfo("*** IP CHECK TRUE: " .. host .. " ***")
			else
				...
				limit = -1  -- this tells the function to read to the end of the string
				...
				repeat
					...
					if count == 0 then
						...
						root = string.lower(string.sub(host, found[count], limit))
						if tld[root] then
							count = count - 1
							limit = found[count + 1] - 2
							loopagain = true
						else
							domain = string.sub(host, found[count])
						end
					end
				until loopagain == false
			
				if domain then
					nw.createMeta(self.keys["root.host"], domain)
					--nw.logInfo("*** META: " .. domain .. " ***")
				end		
			end
...
rootdomain:setCallbacks({
    [nwevents.OnSessionBegin] = rootdomain.sessionBegin,
    [nwlanguagekey.create("alias.host")] = rootdomain.hostMeta,
})
